...
export function ThemeColors({themes}:{themes:ReturnType<typeof useThemes>}){
 ...
 return <section className="sound-character"><h3>Theme colors · {theme.name}</h3><div className="character-knobs">{controls(theme.id,{accent:theme.accent,secondary:theme.secondary,...instrumentDefaults})}</div><button onClick={()=>themes.resetColors(theme.id)}>Reset instrument colors</button><h3>Atmosphere colors</h3><div className="character-knobs">{controls(themes.atmosphereId+'-scene',scene.palette)}</div><button onClick={()=>themes.resetColors(themes.atmosphereId+'-scene')}>Reset atmosphere colors</button><p>Colors are saved separately for each theme.</p></section>;
}
