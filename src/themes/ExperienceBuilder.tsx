...
interface Props {themes:ReturnType<typeof useThemes>;audio:ReturnType<typeof useAudioEngine>;userPatches:EnginePatch[];onSelect:(p:EnginePatch)=>void;onLoadCombination:(skinId:string,p:EnginePatch)=>void;}
export function ExperienceBuilder({themes,audio,userPatches,onSelect,onLoadCombination}:Props) {
 ...
 <div className="theme-builder-layout"><div className="theme-builder-preview"><ThemeStage theme={skin} state={state} simulation={simulation} redline={config.redline} unit="mph" demo motion running/>
  ...
  {themes.combinations.map(c=><div className="combination-row" key={c.id}><button className="forge-text-button" onClick={()=>{onLoadCombination(c.skinId,c.sound);setMessage(`Loaded ${c.name}`);}}>{c.name}<small>{themeForId(c.skinId).name} · {c.sound.name}</small></button><button className="forge-icon" aria-label={`Delete ${c.name}`} onClick={()=>themes.removeCombination(c.id)}>×</button></div>)}
  </section>
 </div><section className="theme-builder-panel"><h2>01 / Appearance</h2><ThemePicker selected={themes.skinId} onSelect={themes.selectSkin}/></section></div></main></div>;
}
