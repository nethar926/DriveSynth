...
import './themes.css';
interface Props {theme:ThemePreset; state:Simulation; simulation:{current:Simulation}; redline:number; unit:'mph'|'kph'; demo:boolean; motion:boolean; running:boolean; gpsLabel?:string;}
...
export function ThemeStage({theme,state,simulation,redline,unit,demo,motion,running,gpsLabel='GPS waiting'}:Props) {
 const speed=state.speedMps*(unit==='kph'?3.6:2.236936), rpm=running?state.rpm:0;
 const rev=Math.max(0,Math.min(1,rpm/redline)), speedPct=Math.min(1,speed/(unit==='kph'?260:160));
 const spaceFont=['tie','xwing'].includes(theme.id);
 const style={'--skin-accent':theme.accent,'--skin-secondary':theme.secondary,'--rev':rev,'--speed':speedPct,'--flow-time':`${Math.max(.4,3-state.speedMps/25)}s`} as CSSProperties;
 ...
 return <div className={`theme-stage layout-${theme.layout} theme-${theme.id} ${spaceFont?'galactic-type':''} ${!motion?'motion-off':''}`} style={style} data-theme-id={theme.id} data-skin={theme.id==='f14'?'aerospace-f14':undefined}>
  {theme.layout==='road'&&<><SceneCanvas scene={sceneForId(theme.sceneId!)} simulation={simulation} motion={motion}/><div className="road-atmosphere"/>{['lofi','neon-drive','miami','alpine'].includes(theme.sceneId!)&&<div className={`road-weather ${theme.sceneId==='alpine'?'snow':''}`} aria-hidden="true"/>}<div className="road-stream" aria-hidden="true"/></>}
  ...
  <div className="skin-body">
   ...
   {theme.layout==='analog'&&theme.id!=='f14'&&<><div className="skin-twin-dials"><Dial value={speed} max={unit==='kph'?240:160} label="SPEED" unit={unit}/><div className="dial-center">{hero}{theme.family==='Cockpit'?radar:<span className={`shift-telltale ${rev>.9?'lit':''}`}>SHIFT</span>}</div><Dial value={rpm/1000} max={redline/1000} label="RPM" unit="× 1000"/></div>{telemetry}<Rail value={rev} label="ENGINE"/></>}
   {theme.layout==='digital'&&<><div className="digital-cluster"><div className="digital-bank"><Rail value={state.load} label="LOAD" segmented/><Rail value={rev} label="ENGINE" segmented/></div>{hero}<div className="digital-tach" aria-label="Tachometer">{Array.from({length:20},(_,i)=><i key={i} className={i/20<rev?'lit':''} style={{height:`${25+i*3.6}%`}}/>)}</div></div>{telemetry}<div className="skin-grid-readout"><span>ENGINE MONITOR</span><span>{state.overrun?'OVERRUN':state.shifting?'SHIFT':'STEADY'}</span><span>{Math.round(redline)} REDLINE</span></div></>}
   ...
   {theme.layout==='time'&&<><div className="time-cluster">{hero}<div className="time-circuits"><div><span>ENGINE RPM</span><b>{String(Math.round(rpm)).padStart(5,'0')}</b></div><div><span>DISTANCE · KM</span><b>{(state.distance/1000).toFixed(2).padStart(5,'0')}</b></div><div><span>ENGINE LOAD</span><b>{String(Math.round(state.load*100)).padStart(3,'0')} %</b></div></div></div><div className="flux-core" aria-hidden="true"><i/><i/><i/></div><Rail value={rev} label="FLUX / RPM"/>{telemetry}</>}
   ...
   {theme.layout==='space'&&<><div className="space-viewport" aria-hidden="true"><i/><i/><i/><i/><div className="space-stars"/></div><div className="space-instruments"><div className="reactor-meter"><Rail value={rev} label="REACTOR" segmented/></div>{hero}<div className="reactor-meter"><Rail value={state.load} label="LOAD" segmented/></div></div>{telemetry}<div className="space-lock" aria-hidden="true"><i/><span className="skin-descriptor">{state.shifting?'SHIFT':'ENGINE ONLINE'}</span><i/></div></>}
  </div>
 </div>;
}
