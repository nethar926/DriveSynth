...
export class CharacterEngine implements EngineSynth {
 ...
 private jet:JetLayers;private fx:ProceduralCharacter;private options:Partial<EngineParams>={};private disposed=false;
 private acoustic: BiquadFilterNode;private lockStage:LockStage='none';private lockSfx=false;onLockStageChange?: (s:LockStage)=>void;private legacyGate:GainNode;private base:EngineSynth;private graph:SignalGraph;private dry:GainNode;private raw:GainNode;private graphDesc:SynthNodeDesc[]=[];private running=false;
 constructor(base:EngineSynth,patch:EnginePatch){this.base=base;this.context=base.context;this.output=this.context.createGain();this.output.gain.value=.65;base.output.disconnect();this.raw=this.context.createGain();this.dry=this.context.createGain();this.legacyGate=this.context.createGain();this.acoustic=this.context.createBiquadFilter();this.acoustic.type='lowpass';this.acoustic.Q.value=.55;this.acoustic.frequency.value=6000;base.output.connect(this.acoustic);this.acoustic.connect(this.legacyGate);this.legacyGate.connect(this.raw);this.raw.connect(this.dry);this.dry.connect(this.output);this.graph=new SignalGraph(this.context,this.raw,this.output);this.output.connect(this.context.destination);this.fx=new ProceduralCharacter(this.context,this.raw);this.jet=new JetLayers(this.context,this.raw);this.fromPatch(patch);}
 get id(){return this.base.id;}
 async start(){await this.base.start();if(!this.disposed){this.running=true;this.fx.start();if(this.base.toPatch().kind==='aerospace')this.jet.start();if(this.graphActive()){this.graph.configure(this.graphDesc);this.graph.start();}}}
 stop(){this.running=false;this.lockStage='none';const hidden=typeof document!=='undefined'&&document.visibilityState==='hidden';this.graph.stop(hidden?0:1.8);this.base.stop();this.fx.stop(hidden);this.jet.stop();}
 dispose(){this.disposed=true;this.base.dispose();this.fx.dispose();this.jet.dispose();this.graph.dispose();this.acoustic.disconnect();this.legacyGate.disconnect();this.raw.disconnect();this.dry.disconnect();this.output.disconnect();}
 setDriving(d:DrivingInput){const kind=this.base.toPatch().kind;const rpm=Math.max(0,Math.min(1,d.rpmNorm??d.speed));const load=Math.max(0,Math.min(1,d.throttle));this.acoustic.frequency.setTargetAtTime(kind==='ev-whine'?2600+rpm*6200:850+rpm*4500+load*1500,this.context.currentTime,.09);const next=kind==='scifi'&&this.running?nextLockStage(rpm,this.lockStage):'none';if(next!==this.lockStage){this.lockStage=next;this.onLockStageChange?.(next);if(next==='lock'&&this.lockSfx)this.fx.cue('lock');}this.base.setDriving(d);this.fx.update(d);if(this.base.toPatch().kind==='aerospace')this.jet.update(d);}
 ...
 private configure(){const kind=this.base.toPatch().kind;this.legacyGate.gain.value=kind==='scifi'||kind==='aerospace'?0:1;this.fx.configure(this.getParams(),kind);this.jet.configure({...this.getParams(),jetIdleRpm:this.base.toPatch().revforge?.idleRpm??800});if(kind!=='aerospace')this.jet.stop();else if(this.running)this.jet.start();}
